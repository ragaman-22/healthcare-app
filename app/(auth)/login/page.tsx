...
export default function LoginPage() {
  ...
  async function handleGoogleLogin() {
    ...
    if (error) {
      setError('Googleログインに失敗しました。Supabase側でGoogle OAuthの設定が必要です。')
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-teal-50 to-blue-50 p-4">
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-8 shadow-sm">
          <h2 className="mb-6 text-lg font-semibold text-gray-800">ログイン</h2>
          ...
          <p className="mt-4 text-center text-sm text-gray-500">
            アカウントをお持ちでない方は{' '}
            <Link href="/register" className="text-teal-600 hover:underline">新規登録</Link>
          </p>
        </div>
      </div>
    </div>
